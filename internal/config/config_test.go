...
`
	...
	tests := []struct {
		...
		{"Auth.JWTSecret", cfg.Auth.JWTSecret, "my-secret-key"},
...
func TestEnvironmentVariableOverrides(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "JWTSecret",
			envVars: map[string]string{
				"KUBEVISION_JWT_SECRET": "env-jwt-secret",
			},
			check: func(t *testing.T, cfg *Config) {
...
func TestAutoGeneratedSecrets(t *testing.T) {
	...
	t.Run("ProvidedSecretNotOverwritten", func(t *testing.T) {
		t.Setenv("KUBEVISION_JWT_SECRET", "user-provided-secret")
		...
		if cfg.Auth.JWTSecret != "user-provided-secret" {
			t.Errorf("Auth.JWTSecret = %q, want %q", cfg.Auth.JWTSecret, "user-provided-secret")
		}
