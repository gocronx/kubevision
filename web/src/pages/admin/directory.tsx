...
export function DirectorySettingsPage() {
  ...
  if (!admin) return <p className="text-sm text-muted-foreground">{t("directory.adminRequired")}</p>
  ...
  return <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
    ...
    {loadError && <div role="alert" className="flex flex-col gap-3 rounded-md border border-destructive/50 bg-destructive/5 p-3 text-sm text-destructive sm:flex-row sm:items-center sm:justify-between"><span>{t("directory.loadFailed")}</span><Button className="self-start" size="sm" variant="outline" onClick={loadSettings}>{t("directory.retry")}</Button></div>}
    ...
    <Card><CardHeader className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between"><CardTitle className="text-base">{t("directory.mappings")}</CardTitle><Button size="sm" variant="outline" onClick={()=>set("mappings",[...form.mappings,{groupId:"",role:"viewer",priority:(form.mappings.at(-1)?.priority ?? 0)+10}])}><Plus/>{t("directory.addMapping")}</Button></CardHeader><CardContent className="space-y-3">{form.mappings.map((mapping,index)=><div key={index} className="grid grid-cols-[minmax(0,1fr)_2.5rem] gap-2 sm:grid-cols-[minmax(0,1fr)_9rem_7rem_2.5rem]"><Input value={mapping.groupId} placeholder={t("directory.groupIdentifier")} onChange={(e)=>set("mappings",form.mappings.map((m,i)=>i===index?{...m,groupId:e.target.value}:m))}/><select className="col-start-1 h-9 rounded-md border bg-background px-2 text-sm sm:col-start-auto" value={mapping.role} onChange={(e)=>set("mappings",form.mappings.map((m,i)=>i===index?{...m,role:e.target.value}:m))}>{roles.map((role)=><option key={role} value={role}>{roleLabels[role]}</option>)}</select><Input className="col-start-1 sm:col-start-auto" type="number" value={mapping.priority} onChange={(e)=>set("mappings",form.mappings.map((m,i)=>i===index?{...m,priority:Number(e.target.value)}:m))}/><Button className="col-start-2 row-start-1 sm:col-start-auto sm:row-start-auto" size="icon" variant="ghost" title={t("directory.removeMapping")} onClick={()=>set("mappings",form.mappings.filter((_,i)=>i!==index))}><Trash2/></Button></div>)}</CardContent></Card>
    <Card><CardHeader><CardTitle className="text-base">{t("directory.previewTitle")}</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex flex-col gap-2 sm:flex-row"><Input value={previewID} onChange={(e)=>setPreviewID(e.target.value)} placeholder={t("directory.userIdentifier")}/><Button variant="outline" onClick={runPreview} disabled={!previewID}><Search/>{t("directory.preview")}</Button></div>{preview&&<div className="rounded-md border p-3 text-sm"><p>{t("directory.resultingRole")}<strong>{roleLabels[preview.role] ?? preview.role}</strong></p><p className="mt-1 break-words text-muted-foreground">{t("directory.groups")}{preview.groups.join(", ") || t("directory.none")}</p></div>}</CardContent></Card>
    <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button variant="outline" onClick={test}><FlaskConical/>{t("directory.test")}</Button><Button onClick={save}><Save/>{t("directory.save")}</Button></div>
  </div>
}
